...
Moderate LOG (<?=isim($usr['Username'].$usr['Tag'],"o")?>)
</div>
...
<?php
$arr = $db->query("SELECT * FROM topicm where handled = '1' and hwho = '".$id."'")->fetchAll(PDO::FETCH_ASSOC);
foreach($arr as $x){
?>			
                                        ...
                                        <tr class="text-center">
											...
											<td>
<button class="badge outline-badge-arbitre pointer" onclick="degistirload('message_name','../ajax/user.php?v=<?=$x['player']?>');degistirload('message_text','../ajax/topicm.php?v=<?=$x['id']?>');" data-toggle="modal" data-target="#exampleModalCenter"> <?=$x['hreason']?></button>
											</td>
											
                                            <td><?=date("d/m/Y H:i:s",($x['date']))?></td>
                                        </tr>
 <?php
}
?>	                                      
